...
export default function SendEmailPage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      sendEmailOrganizerMutation.mutate(values);
    },
  });

  const pending = sendEmailOrganizerMutation.isPending;
  const hasErr = !!(formik.touched.email && formik.errors.email);

  const helpText = useMemo(
    () =>
      sendEmailOrganizerMutation.isSuccess
        ...
        <Card className="w-full max-w-md border-blue-100">
          ...
          <CardContent className="pt-6">
            <form onSubmit={formik.handleSubmit} className="space-y-4" aria-busy={pending}>
              ...
              {/* Server states (opsional) */}
              {sendEmailOrganizerMutation.isError && (
